...
const MessageService = () => {
    ...
    const getMessageByIdCreatedAtJoinWithUsername = (channelId, startDate, endDate) => {
        return sequlize.query(`SELECT message_id,chat_messages.user_id,user_name,message_type,message,filelink,chat_messages.created_at,chat_messages.updated_at FROM chat_messages LEFT JOIN users ON chat_messages.user_id = users.user_id WHERE channel_id = ${channelId} AND message_status = 1 AND created_at BETWEEN "${startDate}" AND ${endDate} ORDER BY 'message_id'  DESC`, { type: sequlize.QueryTypes.SELECT })
    }
    ...
    return {
        ...
        getMessageByIdCreatedAtJoinWithUsername,
